...
public class Menu{
    private static void load(LinkedList<Person> list) {
    ...
    private static void store(LinkedList<Person> list) {
        File obj = new File("ContactList.txt");
        obj.delete();
        try {
            obj.createNewFile();
            FileWriter myObj = new FileWriter("ContactList.txt");
            LinkedList<Person>.Cursor cur = list.connect();
            cur.setAtBeginning();
            while (cur.hasNext()) {
    ...
    private static Person addNewContactMenu() {
        ...
        while (!flag) {
            ...
            flag = PatternChecker.checkName(firstName);
            if (!flag) {
        ...
        while (!flag) {
            ...
            flag = PatternChecker.checkName(lastName);
            if (!flag) {
        ...
        while (!flag) {
            ...
            flag = PatternChecker.checkNumber(contactNumber);
            if (!flag) {
        ...
        while (ack.equals("y") && count++ != 1) {
            ...
            if (ack.equals("y")) {
                ...
                while (!flag) {
                    ...
                    flag = PatternChecker.checkEmail(email);
                    if (!flag) {
    ...
    private static void viewAllContacts(LinkedList<Person> list) {
    ...
    private static void searchForAContactMenu(LinkedList<Person> list) {
        pattern();
    ...
    private static void deleteAContactMenu(LinkedList<Person> list) {
        pattern();
        System.out.println("Here are all your contacts:");
        LinkedList<Person>.Cursor cur = list.connect();
        cur.setAtBeginning();
